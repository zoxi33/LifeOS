...
function LogSleepDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent style={{ background: 'var(--lo-surface)', border: '1px solid var(--lo-border)', borderRadius: 14, maxWidth: 400 }}>
        <DialogHeader>
          <DialogTitle style={{ fontSize: 18, fontWeight: 500 }}>Zaloguj sen</DialogTitle>
        </DialogHeader>
...
export function SleepScreen({ initialDays = [] }: { initialDays?: SleepDay[] }) {
  ...
  const stats = useMemo(() => {
    ...
    const avg = initialDays.reduce((s, d) => s + d.hours, 0) / initialDays.length;
    const consistencyDays = initialDays.filter(d => d.hours >= TARGET_HOURS).length;
    const consistencyPct = Math.round((consistencyDays / initialDays.length) * 100);
    const best = Math.max(...initialDays.map(d => d.hours));
    const avgBed = initialDays.reduce((s, d) => s + d.bed, 0) / initialDays.length;

    const qualityCounts = [0, 0, 0, 0, 0]; // indices 0-4 = quality 1-5
    initialDays.forEach(d => { if (d.quality >= 1 && d.quality <= 5) qualityCounts[d.quality - 1]++; });
    ...
    <>
      ...
      <div className="lo-screen" style={{
        ...
        <div style={{
          ...
          <SectionHeader eyebrow={`Długość snu · ${initialDays.length} dni`} title="Codzienne logi" />
