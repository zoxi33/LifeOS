...
export interface DayData {
  ...
  mood: number | null;
  checklist: { id: string; name: string; done: boolean }[];
}
...
export async function getWeekData(weekOffset: number): Promise<{
  ...
  const [habitsRes, logsRes, focusRes, sleepRes, weightRes, journalRes, checklistDefsRes] = await Promise.all([
    ...
    sb.from('daily_logs').select('date, focus_minutes, checklist').gte('date', start).lte('date', end),
    ...
    sb.from('checklist_items').select('id, name').eq('active', true).order('sort_order').order('created_at'),
  ]);

  const habits = habitsRes.data ?? [];
  const logs = logsRes.data ?? [];
  const focusMap = Object.fromEntries((focusRes.data ?? []).map(r => [String(r.date), { minutes: r.focus_minutes, checklist: (r.checklist ?? {}) as Record<string, boolean> }]));
  ...
  const checklistDefs: ChecklistItemDef[] = (checklistDefsRes.data ?? []).map(r => ({ id: r.id, name: r.name }));
  ...
  const week = dates.map(date => {
    ...
    return {
      ...
      checklist: checklistDefs.map(item => ({ id: item.id, name: item.name, done: checklistDone[item.id] ?? false })),
    };
  });

  return { week, checklistDefs };
}
